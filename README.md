...
* __URL_HOST_ALIASES__: A JSON object mapping a hostname to the one replacing it on submitted URLs, e.g. `{"yt.example.com": "www.youtube.com"}`. Only the host changes, path and query are kept as-is, so a link copied from a self-hosted frontend can be pasted unchanged. Defaults to `{}`.
